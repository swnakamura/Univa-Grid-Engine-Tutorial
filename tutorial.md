...
* `-u <ユーザー名>`オプションをつけると、そのユーザーのジョブの一覧が表示されます。
    *  `qstat -u '*'`とすると、全てのユーザーのジョブの一覧が表示されます。`''`は必須です。
...
その場合、正規表現も使うことができるので利用してもいいでしょう。例えば、`qsub -q 'woody*'`とすると`woody`から始まるサーバー名のもののいずれかに入ることができます。`''`は必須です。
...
*   インストールされているCUDAのバージョンはいくつでしょうか。
    *   実行できない場合は`/usr/local/cuda/bin/nvcc --version`を試してみてください。
